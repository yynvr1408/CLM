"""SLA and renewal API endpoints."""
from fastapi import APIRouter, Depends, HTTPException, status, Query
from sqlalchemy.orm import Session
from app.database.session import get_db
from app.schemas.schemas import (
    RenewalCreate, RenewalResponse, RenewalUpdate
)
...
def create_renewal(
    ...
    renewal = SLAService.create_renewal(db, contract_id, renewal_data)
    return renewal
...
def get_contract_renewals(
    ...
    renewals, total = SLAService.get_contract_renewals(
        db,
...
def get_upcoming_renewals(
    ...
    renewals, total = SLAService.get_upcoming_renewals(
        db,
...
def get_overdue_renewals(
    ...
    renewals, total = SLAService.get_overdue_renewals(
        db,
        skip=skip,
        limit=limit
    )
...
def mark_renewed(
    ...
    renewal = SLAService.mark_renewal_renewed(db, renewal_id, current_user.id)
    return renewal
...
def mark_notified(
    ...
    renewal = SLAService.mark_renewal_notified(db, renewal_id)
    return renewal
...
def update_renewal(
    ...
    renewal = SLAService.update_renewal(db, renewal_id, renewal_data)
    return renewal
